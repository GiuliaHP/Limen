...
B0=blocal_all(0); B16=blocal_all(16)
worst=0;n=0;nmatch=0;wb=""
for bone in defbones:
    up=upath(bone)
    if up not in ub or up not in u16: continue
    dU=trs(ub[up]).inverted()@trs(u16[up])
    dB=B0[bone].inverted()@B16[bone]
    aU=math.degrees(dU.to_quaternion().angle); aB=math.degrees(dB.to_quaternion().angle)
    n+=1
    if abs(aU-aB)<0.5: nmatch+=1
    if abs(aU-aB)>worst: worst=abs(aU-aB); wb=bone
print(f"RES ref FRAME0 : {nmatch}/{n} os angle delta match (<0.5°), pire écart={worst:.3f}° ({wb})")
